...
function configReload(shareList,islast) {
    ...
    var rri =  Math.ceil(Math.random() * 10)+1;
    var shareArr = [timestamp1,'in_in_in',shareList.id,video_id]
    var shareKey  = btoa(JSON.stringify(shareArr))
    shareKey = shareKey+'/'
    // if(shareKey.substring(shareArr.length-2) == '0='){
    //     shareKey = shareKey+'/'
    // }else{
    //     shareKey = shareKey+'0=/'
    // }
        ...
        if (type == 3) {
            ...
            wx.onMenuShareAppMessage({
                ...
                success: function () {
                    ...
                    if(shareList.is_back==0||shareList.is_back=='0'){
                        $('.back-box').hide()
                    }else{
                        $('.back-box').css('display','flex')
                    }
                },
                cancel: function () {
                }
...
function share_tip(share_app_times, share_timeline_times) {
    ...
    if(shareList.share_type == 3){
        ...
        if(shareATimes == 0){
            wxalert(share_wenan, '好')
        }else{
            wenanAlert(shareList.id)
        }
        // for(var i = 0 ;i<share_wenan.length ; i++){
        //     if(share_wenan[i][id] == shareList.id){
        //         wxalert(share_wenan[i][wenan], '好')
        //     }
        // }
